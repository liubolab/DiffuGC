...
def extract(a, t, x_shape):   # a (500)  t (128)  alpha是以cosine递减的，t是从500步中随机选了128个
    b, *_ = t.shape 
    out = a.gather(-1, t)
    return out.reshape(b, *((1,) * (len(x_shape) - 1)))  # 从alpha中，拿出t步的alpha值
...
class GCInferenceLayer(nn.Module):
    def __init__(self,c_in ,d_model, dropout=0.1, max_len=1024):
        super(GCInferenceLayer, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.pe = nn.Parameter(torch.empty(1, max_len, d_model))  # requires_grad automatically set to True
        nn.init.uniform_(self.pe, -0.02, 0.02)
        self.gc_embedding = nn.Linear(c_in, d_model)
    ...
    def forward(self, x):
        ...
        Shape:
            ...
            output: [batch size, sequence length, embed dim]
        """
        x = self.gc_embedding(x) + self.pe[:, :x.shape[1], :]
        return self.dropout(x)
...
# 网络的每一层都能根据当前的噪声水平（时间步）动态调整其行为，使同一网络能够处理不同降噪阶段的输入。
